...
export default function Login() {
  ...
  const goAfterAuth = (userId) => {
    const lastChatId = localStorage.getItem("lastChatId:" + userId);
    if (lastChatId) {
  ...
  const onGoogle = async (cred) => {
    try {
      const data = await googleLogin(cred.credential);
      goAfterAuth(data.user._id);
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#111b21] flex items-center justify-center p-5">
      <div className="w-full max-w-md">
        ...
        <form onSubmit={submit} className="bg-white rounded-xl p-7 shadow-2xl">
          <h2 className="text-xl font-semibold text-[#111b21] mb-5">Sign in</h2>
          ...
          <p className="text-center text-sm text-wa-time mt-5">
            New here?{" "}
            <Link to="/register" className="text-wa-green font-semibold">
              Create account
            </Link>
          </p>
        </form>
